window.helpContent = {
    ...
    gettingstarted: {
        ...
        intro: [
            ...
            "Avis mode en ligne (<a href=\"https://huggingface.co/spaces/Infinitai/Paiperwork\" target=\"_blank\" rel=\"noopener noreferrer\">Hugginface spaces</a>) : En raison des exigences locales, les onglets Documents, Translate, Models et Connectors sont desactives en mode en ligne. Ces onglets sont actives lorsque vous executez Paiperwork sur votre ordinateur."
        ],
        articles: [
            {
                id: "gs-welcome",
        ...
        articles: [
            {
                ...
                content: `
                    ...
                    <ul>
                        <li><strong>Gemma4</strong> - Le dernier modèle visuel de Google avec une forte compréhension des images et un raisonnement conscient du code</li>
                        <li><strong>Qwen3.5</strong> - Modèle de vision haute performance avec d'excellentes capacités multimodales</li>
                        <li><strong>Qwen3.6</strong> - Modèle de vision avancé avec une meilleure gestion du design, de la mise en page et du texte</li>
                        <li><strong>Kimi</strong> - Modèle efficace compatible vision pour des aperçus de design rapides et des workflows sensibles aux images</li>
    ...
    connectors: {
        title: "Connecteurs",
        intro: [
            "Remarque : la version en ligne de Paiperwork hébergée sur Huggingface n'inclut pas la fonctionnalité WhatsApp ou WeChat pour des raisons de sécurité et de confidentialité des utilisateurs.",
            "Langues prises en charge par l'orchestrateur WhatsApp/WeChat : anglais, espagnol, portugais, allemand, chinois, français, japonais, coréen et russe.",
            "Les connecteurs relient Paiperwork à WhatsApp/WeChat. Les requêtes n'utilisent que les ressources déjà disponibles dans Paiperwork et n'accèdent pas à votre système d'exploitation, à la mémoire, au disque dur ni à des fichiers externes arbitraires.",
            "Pour recevoir les messages WhatsApp/WeChat entrants, gardez actif l'onglet du navigateur qui exécute Paiperwork et empêchez l'ordinateur de se mettre en veille. L'écran de verrouillage interrompra les messages entrants ; vous pouvez envisager de le désactiver temporairement pour activer les fonctionnalités WhatsApp/WeChat.",
            ...
            "Vous pouvez créer des présentations et des mini-apps à partir des résumés de documents et de leurs modifications de suivi, des rapports de recherche et de leurs modifications de suivi, ainsi que des entrées de base de connaissances et de leurs modifications de suivi."
        ],
        articles: [
            {
                id: "connectors-pairing",
                title: "Appairage et Modes",
                content: `
                <p>WhatsApp : Ouvrez l'onglet Connecteurs et choisissez d'abord un mode avant de démarrer le serveur.</p>
                <ol>
                    <li><strong>Mode personnel :</strong> vous connectez votre propre compte WhatsApp et vous discutez avec vous-même dans un flux privé.</li>
                ...
                <p>WeChat : pour apparier le bot WeChat, cliquez sur le bouton Start server, scannez le code QR et suivez les instructions dans l’application WeChat de votre téléphone. Les démarrages suivants du serveur conserveront les mêmes informations d’appariement, sauf si vous supprimez l’appareil appairé.</p>
                <h4>Effacer les contextes WhatsApp/WeChat</h4>
                <p>Si les réponses WhatsApp/WeChat deviennent trop chargées après une longue utilisation, vous pouvez effacer uniquement les conversations WhatsApp/WeChat par téléphone stockées dans la base de données sans supprimer le reste de votre base de données ; les conversations liées à Paiperwork ne seront pas supprimées.</p>
                <ol>
                    <li>Cliquez sur "Effacer les contextes WhatsApp/WeChat" dans l'onglet Connecteurs</li>
                    <li>Cela supprime la mémoire de contexte WhatsApp/WeChat stockée par téléphone et réinitialise le contexte WhatsApp/WeChat actif en mémoire</li>
                ...
                <p>Si vous souhaitez supprimer complètement le lien avec Paiperwork, faites-le depuis votre téléphone dans WhatsApp/WeChat sous <strong>Linked devices</strong>.</p>
            `,
            },
            {
                id: "connectors-models-chat",
                title: "Sélection du Modèle et Chat",
                content: `
                <p>Une fois connecté, WhatsApp utilise le modèle actuellement sélectionné dans l'onglet Chat.</p>
                <p>Vous pouvez vérifier quel modèle d'IA est actif et le changer directement depuis votre discussion WhatsApp.</p>
                <p>Pour entrer d'abord dans le mode de contrôle des modèles, envoyez <code>mode modèles</code>, <code>mode modeles</code>, <code>mode modèle</code> ou <code>mode modele</code>.</p>
                <h4>Commandes utiles pour les modèles</h4>
                <ul>
                    <li><code>quel modèle est sélectionné</code></li>
                    <li><code>montre-moi mes modèles</code></li>
                ...
                <ul>
                    ...
                    <li><code>explique la différence entre OLED et Mini LED</code></li>
                </ul>
            `,
            },
            {
                id: "connectors-workflows",
                title: "Documents, Graphiques, Recherche, Présentations et Artefacts",
                content: `
                <p>Les flux du connecteur restent en chat normal jusqu'à ce que vous entriez explicitement dans un mode. Utilisez les mots-clés ci-dessous dans chaque section.</p>
                <h4>Internet</h4>
                <p>Utilisez le mode Internet lorsque vous voulez que les réponses de chat ordinaires s'appuient sur une recherche web en direct au lieu de répondre uniquement avec les connaissances locales du modèle.</p>
                <p>Entrez dans ce mode avec <code>mode internet</code>, <code>mode web</code>, <code>mode recherche web</code> ou <code>mode en ligne</code>.</p>
                <ul>
                    <li><code>Quelles sont les dernières nouvelles sur la guerre entre l'Iran et les États-Unis ?</code></li>
                    <li><code>Quel temps fait-il aujourd'hui à mon emplacement ?</code></li>
                    <li><code>Cherche sur internet la météo d'aujourd'hui à Guangzhou</code></li>
                </ul>

                <h4>Documents</h4>
                <p>Vous pouvez poser des questions sur un document importé ou demander un résumé. Les noms de documents n'ont pas besoin d'être exacts.</p>
                <p>Entrez dans ce mode avec <code>mode document</code> ou <code>mode documents</code>.</p>
                <p>Cette fonction nécessite qu'un modèle d'embeddings local soit installé et sélectionné dans l'onglet Documents.</p>
                <ul>
                    <li><code>parcourir documents</code></li>
                    <li><code>je veux voir un document mais je ne me souviens pas du nom</code></li>
                    <li><code>je veux poser une question sur un document</code></li>
                    <li><code>résumer &lt;nom du document&gt;</code></li>
                    <li><code>&lt;nom du document&gt; expliquer</code></li>
                    <li><code>&lt;nom du document&gt; résumé et présentation</code></li>
                    <li><code>&lt;nom du document&gt; résumé et mini app</code></li>
                </ul>
                <p>Pour quitter le mode de suivi des documents, utilisez <code>quitter le mode document</code> ou <code>j'ai fini</code>.</p>
                <h5>Interactions</h5>
                ...
                <p><strong>Utilisateur :</strong> <code>Oui, créer une présentation</code></p>
                <p><strong>Paiperwork :</strong> Création d'une présentation SlideForge pilotable...</p>
                <p><strong>Paiperwork :</strong> Présentation créée et envoyée comme fichier HTML.</p>
                <p><strong>Paiperwork :</strong> Voulez-vous apporter d'autres modifications à cette présentation ?</p>
                ...
                <p>Entrez dans ce mode avec <code>mode graphiques</code>, <code>mode graphique</code>, <code>mode dataviz</code> ou <code>mode visualisation</code>.</p>
                <ul>
                    <li><code>créer un pie chart</code></li>
                    <li><code>créer un radar chart</code></li>
                    <li><code>créer un bar chart sur les ventes par trimestre</code></li>
                </ul>
                <p>Fournissez vos propres données si vous voulez un vrai graphique au lieu d'une démo.</p>

                <h4>Recherche</h4>
                <p>Vous pouvez demander à Paiperwork d'effectuer une recherche sur un sujet pour vous. Les rapports standard font généralement entre 1000 et 1500 mots, cela peut donc prendre un peu de temps.</p>
                <p>Entrez dans ce mode avec <code>mode recherche</code>.</p>
                <ul>
                    <li><code>recherche les dernières tendances des batteries pour véhicules électriques et résumer les opportunités pour les startups</code></li>
                    <li><code>recherche les prix des logements en australie</code></li>
                    <li><code>créer un rapport sur la météo en angleterre</code></li>
                    <li><code>ajoute aussi l'impact de la météo sur la fréquentation des plages</code></li>
                </ul>
                <p>Pour fermer le mode de suivi de recherche, répondez <code>non</code>, <code>non merci</code> ou <code>j'ai fini</code>.</p>
                <h5>Interactions</h5>
                <p><strong>Paiperwork :</strong> La recherche a commencé pour <code>&lt;sujet de recherche&gt;</code>. Collecte des informations...</p>
                <p><strong>Paiperwork :</strong> Quand vous avez terminé, répondez avec <code>non</code>, <code>non merci</code> ou <code>j'ai fini</code> pour fermer le mode recherche.</p>
                <p><strong>Paiperwork :</strong> Voulez-vous continuer à affiner cette recherche ?</p>
                <p><strong>Utilisateur :</strong> <code>Ajoute &lt;raffinement de recherche supplémentaire&gt;</code></p>
                <p><strong>Paiperwork :</strong> La recherche avec raffinement supplémentaire a commencé.</p>
                <p><strong>Paiperwork :</strong> Recherche affinée livrée.</p>
                <p><strong>Paiperwork :</strong> Voulez-vous continuer à affiner cette recherche ?</p>
                ...
                <p>Entrez dans ce mode avec <code>mode présentation</code>, <code>mode presentation</code>, <code>mode présentations</code>, <code>mode presentations</code> ou <code>mode diapositives</code>.</p>
                ...
                <div class="note"><p><strong>Remarque :</strong> Les versions Wechat pour téléphone et tablette restreignent l'exécution de code pour des raisons de sécurité ; les présentations risquent donc de ne pas s'y afficher. Utilisez l'application Wechat sur l'ordinateur afin de pouvoir les télécharger sur votre ordinateur et les voir correctement (assurez-vous d'enregistrer le fichier en dehors du dossier propre à Wechat).</p></div>
                <div class="note"><p><strong>Remarque :</strong> Une fois le fichier HTML de présentation téléchargé sur votre ordinateur, vous pouvez le modifier hors ligne avec la petite barre d'outils en haut à droite. Les téléphones et tablettes ne sont pas pris en charge pour ce flux d'édition. Utilisez <strong>Edit text</strong> pour modifier le texte directement, cliquez sur n'importe quelle image de la présentation pour la remplacer par une image locale, utilisez <strong>Fullscreen</strong> pour afficher la présentation en plein écran et utilisez <strong>Save</strong> pour enregistrer le résultat mis à jour comme nouveau fichier HTML. La barre d'outils disparaît automatiquement lorsque la présentation est en plein écran.</p></div>
                <ul>
                    <li><code>avec le texte suivant créer une présentation : &lt;texte de présentation&gt;</code></li>
                    <li><code>avec ce texte créer une présentation : &lt;texte de présentation&gt;</code></li>
                    <li><code>crée-moi des slides sur la politique européenne des véhicules électriques, utilise la recherche web, 7 slides</code></li>
                    <li><code>liste mes présentations</code></li>
                    <li><code>envoie-moi &lt;nom de la présentation&gt;</code></li>
                </ul>
                <h5>Interactions</h5>
                <p><strong>Utilisateur :</strong> <code>Crée une présentation sur le système solaire et ses planètes, cherche en ligne.</code></p>
                <p><strong>Paiperwork :</strong> Création d'une présentation SlideForge pilotable avec recherche web...</p>
                <p><strong>Paiperwork :</strong> Présentation créée et envoyée comme fichier HTML.</p>
                <p><strong>Paiperwork :</strong> Voulez-vous apporter d'autres modifications à cette présentation ?</p>
                <p><strong>Utilisateur :</strong> <code>Oui, change le fond de la diapositive 3 en thème bleu</code></p>

                <h4>Artefacts</h4>
                <p>Vous pouvez demander directement depuis WhatsApp des artefacts ou mini-apps HTML de toutes sortes.</p>
                <p>Entrez dans ce mode avec <code>mode mini app</code>, <code>mode miniapp</code>, <code>mode mini application</code> ou <code>mode artefact</code>.</p>
                <div class="note"><p><strong>Remarque :</strong> Les applications WhatsApp sur téléphone, tablette et ordinateur limitent l'exécution de code pour des raisons de sécurité ; les mini-apps risquent donc de ne pas s'y afficher ou fonctionner correctement. Utilisez WhatsApp Web afin de pouvoir les télécharger sur votre ordinateur et les utiliser correctement.</p></div>
                <div class="note"><p><strong>Remarque :</strong> Les versions Wechat pour téléphone et tablette restreignent l'exécution de code pour des raisons de sécurité ; les mini-apps risquent donc de ne pas s'y afficher ou fonctionner correctement. Utilisez l'application Wechat sur l'ordinateur afin de pouvoir les télécharger sur votre ordinateur et les utiliser correctement (assurez-vous d'enregistrer le fichier en dehors du dossier propre à Wechat).</p></div>
                <ul>
                    <li><code>crée une mini app de fond d'écran animé relaxant avec différents bruits et des oscillateurs de basse fréquence, avec recherche web</code></li>
                    <li><code>créer artefact galaga</code></li>
                    <li><code>crée une mini app pour trouver des chaînes tv en ligne</code></li>
                    <li><code>créer un artefact mp3 player</code></li>
                    <li><code>montre-moi mes mini apps</code></li>
                    <li><code>montre-moi mes artefacts</code></li>
                    <li><code>envoie-moi &lt;nom de l'artefact&gt; mini app</code></li>
                    <li><code>envoie-moi &lt;nom de l'artefact&gt; artefact</code></li>
                </ul>
                <h5>Interactions</h5>
                <p><strong>Paiperwork :</strong> Voulez-vous apporter d'autres modifications à cette mini app ?</p>
                <p><strong>Utilisateur :</strong> <code>Oui, change le fond</code></p>
                <p><strong>Paiperwork :</strong> Voulez-vous apporter d'autres modifications à cette mini app ?</p>
                <p><strong>Utilisateur :</strong> <code>Non</code></p>
                ...
                <p>Pour quitter un mode explicite et revenir au chat normal, utilisez <code>quitter le mode</code>, <code>fermer le mode</code>, <code>désactiver le mode</code> ou <code>desactiver le mode</code>.</p>
            ...
            {
                id: "connectors-notes",
                title: "Notes de Fonctionnement et Limites",
                content: `
                <ul>
                    ...
                    <li>Paiperwork reste en chat normal par défaut. Les flux documents, graphiques, modèles, recherche, présentations et mini-apps ne démarrent qu'après l'envoi de leurs mots-clés de mode explicites.</li>
                    <li>Si vous revenez de la page de génération à la page d'accueil ou si vous actualisez le navigateur, le serveur WhatsApp/WeChat se ferme. Vous devez le redémarrer manuellement pour reprendre la communication sur WhatsApp/WeChat.</li>
                    <li>Les présentations et artefacts générés depuis WhatsApp/WeChat sont renvoyés comme fichiers HTML. Vous pourrez ensuite les ouvrir dans Paiperwork pour des retouches manuelles plus poussées.</li>
                    <li>Lors d'opérations longues comme le résumé de documents, la recherche, la création de présentations ou la génération de mini-apps, vous pouvez toujours annuler à tout moment en envoyant <code>Cancel</code>, <code>Stop</code> ou <code>Exit</code> depuis WhatsApp/WeChat.</li>
                    <li>Par défaut, Paiperwork isole l'état du connecteur WhatsApp/WeChat par utilisateur de Master Key afin que les sessions enregistrées, l'état des appareils et les données d'exécution en file d'attente ne fuient pas entre différents utilisateurs de Master Key sur la même machine.</li>
                </ul>
            `,
            }
        ],
