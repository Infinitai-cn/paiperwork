...
func ValidateJidWithLogin(client *whatsmeow.Client, jid string) (types.JID, error) {
	...
	if config.WhatsappAccountValidation {
		// No need to validate self JID: own chat is always valid.
		if client != nil && client.Store != nil && client.Store.ID != nil {
			if strings.EqualFold(parsedJID.String(), client.Store.ID.String()) {
				return parsedJID, nil
			}
		}

		if !IsOnWhatsapp(client, jid) {
			// Account validation is best-effort; if we cannot confirm due timeout or API failures,
			// do not block messaging entirely (to avoid issues with intermittent gateway issues).
			logrus.Warnf("Whatsapp account validation failed for %s; allowing send as fallback", jid)
			// Keep the route open for now; in the worst case the send operation itself will fail.
			return parsedJID, nil
		}
	}
