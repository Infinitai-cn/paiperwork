...
func (service serviceSend) SendImage(ctx context.Context, request domainSend.ImageRequest) (response domainSend.GenericResponse, err error) {
	...
	if request.ImageURL != nil && *request.ImageURL != "" {
		...
		// Read multipart image into memory
	...
	if config.NoDisk {
		...
		resizedImage := imaging.Resize(processedImage, 100, 0, imaging.Lanczos)
		var thumbBuf bytes.Buffer
		if err = imaging.Encode(&thumbBuf, resizedImage, imaging.JPEG); err != nil {
			return response, pkgError.InternalServerError(fmt.Sprintf("failed to encode thumbnail %v", err))
		}
		thumbnailData = thumbBuf.Bytes()
		imageThumbnail = fmt.Sprintf("inmemory://thumbnail-%s", imageName)

		if request.Compress {
			compressedImg := imaging.Resize(processedImage, 600, 0, imaging.Lanczos)
			var compressedBuf bytes.Buffer
			if err = imaging.Encode(&compressedBuf, compressedImg, imaging.JPEG); err != nil {
				return response, pkgError.InternalServerError(fmt.Sprintf("failed to encode compressed image %v", err))
			}
			imageData = compressedBuf.Bytes()
			imagePath = fmt.Sprintf("inmemory://compressed-%s", imageName)
		} else {
