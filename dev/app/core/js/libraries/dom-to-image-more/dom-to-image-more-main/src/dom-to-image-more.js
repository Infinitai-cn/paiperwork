(function (global) {
    ...
    * @param {String} options.fontEmbedCss - CSS string with @font-face rules to inject into the SVG output.
    ...
    function toSvg(node, options) {
        ...
        function makeSvgDataUri(clone) {
            ...
            const customFontCss = getCustomFontEmbedCss();
            ...
            return Promise.resolve(clone)
                ...
                .then(function (xhtml) {
                    return wrapForeignObjectMarkup(xhtml, customFontCss);
                })
            ...
            function wrapForeignObjectMarkup(xhtml, fontCss) {
                const normalizedMarkup = String(xhtml || '');
                const normalizedFontCss = String(fontCss || '').trim();
                const fontStyleMarkup = normalizedFontCss
                    ? `<style data-domtoimage-font-embed="true">${escapeXmlText(normalizedFontCss)}</style>`
                    : '';
                ...
                if (/^<html[\s>]/i.test(normalizedMarkup)) {
                    ...
                    if (/<head[\s>]/i.test(normalizedMarkup)) {
                        return normalizedMarkup.replace(/<head([^>]*)>/i, `<head$1>${fontStyleMarkup}`);
                    }

                    return normalizedMarkup.replace(/<html([^>]*)>/i, `<html$1><head>${fontStyleMarkup}</head>`);
                }

                return `<html xmlns="http://www.w3.org/1999/xhtml"><head>${fontStyleMarkup}</head><body style="margin:0;padding:0;">${normalizedMarkup}</body></html>`;
            }
        ...
        function getCustomFontEmbedCss() {
            if (typeof options.fontEmbedCss === 'string' && options.fontEmbedCss.trim()) {
    ...
    function copyOptions(options) {
        ...
        domtoimage.impl.options.disableEmbedFonts = Boolean(options.disableEmbedFonts);
        domtoimage.impl.options.fontEmbedCss =
            typeof options.fontEmbedCss === 'string' ? options.fontEmbedCss : '';
        domtoimage.impl.options.fontEmbedCSS =
            typeof options.fontEmbedCSS === 'string' ? options.fontEmbedCSS : '';
    }
    ...
    function embedFonts(node) {
        const customFontCss =
            typeof domtoimage.impl.options.fontEmbedCss === 'string' && domtoimage.impl.options.fontEmbedCss.trim()
                ? domtoimage.impl.options.fontEmbedCss
                : (typeof domtoimage.impl.options.fontEmbedCSS === 'string' && domtoimage.impl.options.fontEmbedCSS.trim()
                    ? domtoimage.impl.options.fontEmbedCSS
                    : '');

        if (customFontCss || domtoimage.impl.options.disableEmbedFonts) {
            return Promise.resolve(node);
        }
