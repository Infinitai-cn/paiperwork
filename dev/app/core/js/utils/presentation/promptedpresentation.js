...
`;
	...
	static async saveHtmlToDisk(title, htmlContent) {
		const standaloneHtml = await this.buildStandalonePromptableHtml(htmlContent || '', null, { includeEditorShell: true });
	...
	static injectStandalonePromptableEditor(documentRef, options = {}) {
		...
		const styleText = [
			...
			'body img{cursor:pointer !important;}',
		...
		toolbar.innerHTML = [
			...
			'<button type="button" data-role="toggle-fullscreen">Fullscreen</button>',
			'<button type="button" class="pw-standalone-editor-save" data-role="save-html">Save</button>',
			'<span class="pw-standalone-editor-hint" data-role="hint">Click the image to replace it</span>'
		].join('');
		...
		script.textContent = [
			...
			'  var textButton = toolbar.querySelector("[data-role=toggle-text-edit]");',
				'  var fullscreenButton = toolbar.querySelector("[data-role=toggle-fullscreen]");',
			...
			'  var setButtonState = function(button, isActive){ if (button) { button.setAttribute("data-active", isActive ? "true" : "false"); } };',
				'  var getFullscreenTarget = function(){ return doc.documentElement || body; };',
				'  var isFullscreenActive = function(){ return !!(doc.fullscreenElement || doc.webkitFullscreenElement || doc.msFullscreenElement); };',
				...
				'      }',
				'    } catch (error) {',
				'      console.error("[PromptablePresentation][standalone-editor] Fullscreen toggle failed", error);',
				'      updateHint("Fullscreen is not available in this browser");',
				'    }',
				...
				'    if (!slideCandidates.length) return null;',
				...
				'    slideCandidates.forEach(function(candidate){',
				'      try {',
				'        var rect = candidate.getBoundingClientRect();',
				'        var overlapTop = Math.max(rect.top, 0);',
				'        var overlapBottom = Math.min(rect.bottom, viewportHeight);',
				'        var overlapLeft = Math.max(rect.left, 0);',
				'        var overlapRight = Math.min(rect.right, viewportWidth);',
				...
				'        var distance = Math.abs(centerY - viewportCenterY) + (Math.abs(centerX - viewportCenterX) * 0.35);',
				...
				'      } catch (_error) {}',
				'    });',
				'    return bestCandidate || slideCandidates[0];',
				'  };',
			...
			'    });',
				...
				'    updateHint(enabled ? "Text editing on" : "Click the image to replace it");',
			'  };',
			'  var buildExportHtml = function(){',
			'    var clone = doc.documentElement.cloneNode(true);',
			'    clone.classList.remove("pw-standalone-editing");',
			...
			'  });',
				'  fullscreenButton && fullscreenButton.addEventListener("click", function(){ void toggleFullscreen(); });',
			...
			'  });',
				...
				'  updateFullscreenButton();',
			'  updateHint("Click the image to replace it");',
			'})();'
		].join('');
